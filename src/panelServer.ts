...
const collectJobs = (): PanelJobSnapshot[] => {
  const storeAccessor = jobStore as unknown as StoreAccessor;

  const rawJobs =
    typeof storeAccessor.getSerializedJobs === 'function'
...
const renderJobsTable = (jobs: PanelJobSnapshot[]): string => {
  const rows = jobs
    .map((job) => {
...
const renderPage = (jobs: PanelJobSnapshot[]): string => {
  const content =
    jobs.length === 0
...
export async function startPanelServer(): Promise<void> {
  const app = express();
